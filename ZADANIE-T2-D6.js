...
uzasadnienie(`tak sie pisze i już`);
...
*/
odpowiedź('B');
uzasadnienie(`Nie przyspiesza działania kodu ale za to ułatwia np niektóre czynności mogą się wykonać w trakcie oczekiwania na inne`);
...
*/
odpowiedź('abc');
uzasadnienie(`abc ponieważ tak do działa - funkcja zmienia element o identyfikatorze test na zmienną str która wyświetli tekst abc - nie będzie to def bo ta linia zmienia tylko wartość zmiennej str a nie to co się będzie wyświetlało`);
...
*/
odpowiedź("A,B,C,D,E");
uzasadnienie(`No generalnie to poczytałem o tym promisie i wszystko wyszło że się zgadza xd`);
...
uzasadnienie(`Tak juz jest było w niedziele na workshopie - tam się podaje funkcje do wywoływania i interwał czasowy w ms`);
...
*/
odpowiedź("B");
uzasadnienie(`Tak wydedukowałem z poprzedniego pytania o asynchroniczność - wiedza że jest tylko jedna poprawna odpowiedz ulatwila zadanie`);
...
*/
odpowiedź("A,B,E");
uzasadnienie(`Te przykłady były w dokumentacji i rozumiem że A - bo odczytanie może trwać i chcemy żeby inne czynności się wykonywały w tym czasie B - to samo, E - bo czeka na akcję użytkownika tak mi sie wydaje. Reszta odp tez musialaby byc asynchroniczna np jakbysmy pobierali obrazek albo tekst z neta`);
...
uzasadnienie(`bylo w niedziele na workshopie`);
...
uzasadnienie(`tez bylo na workshopie - konfident nasluchuje czy akcja zostala wykonana w tym przypadku akcja click - potem wykonuje określone zadanie czyli wyswietla alert`);
...
*/
odpowiedź('CORS');
uzasadnienie(`wygooglowałem xd to jest takie zabezpieczenie`);
